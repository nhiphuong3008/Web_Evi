...
def calculate_real_class_lesson_dates_fixed(schedules, class_name, matched_syllabuses, session):
            ...
            if syl.official_date:
                try:
                    ...
                    if len(p) == 3:
                        ...
                        # MONOTONIC CHECK: Only accept if dt >= last_dt
    ...
    # 4. Forward Fill & Backward Fill around known dates
    ...
    for idx in range(0, total_lessons - 1):
        if lesson_dates[idx + 1] is None:
            next_d = get_next_study_date(lesson_dates[idx], sorted_weekdays)
            buoi_num = (matched_syllabuses[idx + 1].lesson_num if matched_syllabuses else idx + 2)
            if buoi_num in delayed_lessons:
                next_d = get_next_study_date(next_d, sorted_weekdays)
            lesson_dates[idx + 1] = next_d

    return lesson_dates, delayed_lessons

# Audit with fixed date calculator
session = db_session()
...
today = datetime.date.today()
print(f"Auditing all {len(all_classes)} classes with MONOTONIC DATE CALCULATOR...")
print("="*120)
...
for cname in all_classes:
    ...
    lesson_dates, delayed_set = calculate_real_class_lesson_dates_fixed(schedules, cname, matched_syllabuses, session)
    ...
    # Find current active lesson (today or nearest)
    ...
    print(f"{status_tag} {cname:20s} | Total: {len(lesson_dates):2d} | Completed: {comp_count:2d} | Pending: {pend_count:2d} | Current Active: Buổi {curr_buoi:2d} ({curr_date}) | Date Errors: {date_errors}")
